...
export class RolesService {
  ...
  async createRole(roleData: CreateRoleDto): Promise<Role> {
    ...
    try {
      ...
      if (
        err.code === 11000 ||
        (err.name === 'MongoServerError' && err.message?.includes('duplicate key'))
      ) {
  ...
  async updateRolePermissions(roleId: string, permissions: string[]): Promise<Role> {
    try {
      const updatedRole = await this.roleModel
        .findByIdAndUpdate(roleId, { permissions }, { new: true, runValidators: true })
        .exec();
  ...
  async toggleRoleStatus(roleId: string, isActive: boolean): Promise<Role> {
    try {
      const updatedRole = await this.roleModel
        .findByIdAndUpdate(roleId, { isActive }, { new: true })
        .exec();
      ...
      throw new InternalServerErrorException(
        `Failed to ${isActive ? 'activate' : 'deactivate'} role`,
      );
    }
  }
  ...
  async updateRole(roleId: string, updateData: Partial<Role>): Promise<Role> {
    try {
      const updatedRole = await this.roleModel
        .findByIdAndUpdate(roleId, updateData, { new: true, runValidators: true })
        .exec();
      ...
      if (
        ...
        const duplicateName = err?.keyValue?.name || updateData.name;
        throw new ConflictException(`Role with name '${duplicateName}' already exists`);
      }
      // Check for Mongoose validation errors
      if (err.name === 'ValidationError') {
